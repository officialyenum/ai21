...
import { useBody, useHeaders, useSummarize, useSummarizeBody, useRewrite, useRewriteBody, useParaphrase, useParaphraseBody } from './library';
export class AI21 {
    token;
    constructor(token) {
        this.token = token;
    ...
    async paraphrase(data) {
        try {
            const token = this.validate().token;
            const paraphraseData = useParaphrase();
            const body = useParaphraseBody(data);
            const response = await axios.post(paraphraseData.url, body, { headers: useHeaders(token) });
            if (response)
